...
function readJson(p) {
    return fs_1.promises.readFile(p, 'utf8').then(c => JSON.parse(c));
}
function readYaml(fileName) {
    return fs_1.promises.readFile(fileName, 'utf8').then(c => jsYaml.load(c));
}
        ...
        async function unchainedFunction() {
            try {
                ...
                try {
                    ...
                    fileContent = await fs_1.promises.readFile(fileName, { encoding: 'utf8' });
                }
                catch (err) {
                    var error = (0, cast_error_1.expected)(err);
        ...
        if (typeof fileNameOrObject === "string") {
            ...
            async function searchFileName() {
                if (exts.length) {
                    ...
                    var ext = exts.shift();
                    try {
                        await fs_1.promises.access(fileNameOrObject + ext, fs_1.promises.constants.R_OK);
