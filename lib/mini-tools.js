...
MiniTools.serveErr=function serveErr(req,res,next){
    return function(err){
        ...
        MiniTools.serveErr.propertiesWhiteList.forEach(function(attrName){
            if(attrName in err){
                text+='\n'+attrName+': '+JSON.stringify(err[attrName]);
            }
        });
        if(MiniTools.serveErr.sendStack){
            text+='\n------------------\n'+err.stack;
        }
        if(!res.headersSent){
...
MiniTools.serveErr.propertiesWhiteList=['code'];
...
MiniTools.serveTransforming = function serveTransforming(pathToFile, anyFileOrOptions, extOriginal, extTarget, renderizer, textType){
    ...
    if(anyFileOrOptions==null || typeof anyFileOrOptions==="boolean"){
        ...
        console.log('xxxxxxxxxxxxxxxxxx pathToFile',pathToFile,anyFileOrOptions,extOriginal);
